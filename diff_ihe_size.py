...
fluids = ['Isopentane', 'R245CA'] # 'R600', 'R245fa', 'R245CA', 'R11', 'Isopentane', 'n-Pentane', 'R123', 'R141B', 'R113'
...
for fluid in fluids:
    ...
    eff = PowerPlantWithIHE.calculate_efficiency_with_ihe(10, 0, None)
    Q_ihes = np.linspace(-2e6, 0, 20)
#    p_tur = np.linspace(8, 15, 20)
    for Q_ihe in Q_ihes:
        eff2 = PowerPlantWithIHE.calculate_efficiency_with_ihe(None, Q_ihe, 70)
        sensitivity_analysis_Q_ihe.loc[Q_ihe, 'power_output'], \
        sensitivity_analysis_Q_ihe.loc[Q_ihe, 'thermal_efficiency'], \
        sensitivity_analysis_Q_ihe.loc[Q_ihe, 'T_i']=PowerPlantWithIHE.get_results()
