...
#fluids = ['Isopentane'] #, 'Isobutane','R245fa', 'R600', 'R245CA', 'R123', 'Isopentane', 'n-Pentane', 'R113', 'R141B', 'R11'
...
#        Td_bp_conds = np.linspace(34, 35, 10)
...
#        plot_sensitivity_analysis(sensitivity_analysis_without_ihe, fn=fluid, kw='without_ihe')
# ----------------------------------------------------------------------------------------------------------------------
...
class optimization_problem():
    ...
    def fitness(self, x):
        f1 = 1 / self.model.calculate_efficiency_opt(x, working_fluid='R245CA')
        ci1 = -x[0] + x[1]
        print(x)
        return [f1, ci1]
    ...
    def get_bounds(self):
        ...
        return ([20, -0.01], [29, -0.01])

optimize = optimization_problem()
optimize.model = PowerPlant(working_fluid='R245CA')
