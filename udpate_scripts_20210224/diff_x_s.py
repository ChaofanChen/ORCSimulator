...
fluids = ['R113']#'R245fa', 'R600', 'R245CA', 'R123', 'Isopentane', 'n-Pentane', 'R113', 'R141B', 'R11'
for fluid in fluids:
    print('+' * 75)
    ...
    Q_range = -np.linspace(5e7, .5e6, 20)

    PP.nw.get_comp('internal heat exchanger').set_attr(pr1=1, pr2=1)
    for Q in Q_range:
        PP.run_simulation(Q_brine_ev=Q, Q_ihe=0, T_air_hot=15, geo_steam_share=0.15)
    ...
    ax.plot(sensitivity_analysis_without_ihe.index, sensitivity_analysis_without_ihe['power_output'], color='blue', marker="o")
    ax.set(xlabel= 'Turbine inlet pressure with ' + fluid + ' (bar)', ylabel='Gross power output (MW)')
#    plt.ylim(10, 18)
    ax2=ax.twinx()
    ax2.plot(sensitivity_analysis_without_ihe.index, sensitivity_analysis_without_ihe['T_i'], color='black', marker="*", label='Re-injection temperature')
...
#    ax2.plot(np.linspace(10, 26, 5), (70,70,70,70,70), dashes=[2, 2], linewidth=3, color='red')
    ax.grid()
#    ax.legend(loc='upper left')
#    ax2.legend(loc='lower right')
#    plt.show()
    # fig.autofmt_xdate()
    plt.savefig('lower_x_s_with_' + fluid + '.png')
