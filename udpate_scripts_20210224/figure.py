...
for fluid in fluids:
    df = pd.read_csv('diff_p_before_tur_' + fluid + '_without_ihe.csv', 
                         header=0, names=['p_before_tur', 'power_output','thermal_efficiency',
                                          'net_power', 'net_efficiency', 'T_i', 'Q_IHE', 'Q_Brine_EV'])
    fig, ax = plt.subplots(figsize=(8, 6), dpi=300)
    ax.plot(df['p_before_tur'], df['power_output']/1e6, color='blue', marker="o") #, label = 'Gross power output'
#    ax.plot(df['p_before_tur'], df['net_power']/1e6, color='blue', marker="x")
    ax.set(xlabel= 'Turbine inlet pressure with ' + fluid + ' (bar)', ylabel='Gross power output amount (MW)')
#    plt.ylim(14, 19)
    ax2=ax.twinx()
    ax2.plot(df['p_before_tur'], df['T_i'], color='red', marker="*", label = 'Re-injection temperature')
